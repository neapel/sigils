...
%\newcommand{\signDesc}[1]{\signDescFont{\uppercase{#1}}}
...
\newcommand{\sign}[3][\defname]{%
	...
	\draw
		(SignHere)node(SignNameNode)[
			anchor=base, text width=55mm,
			align=center, font=\signNameFont
		]{\uppercase{#1}}

		(SignNameNode.south)node[
			anchor=north,
			align=center, font=\signDescFont
		]{\uppercase{#3}}
		;
}
...
%		\includegraphics[scale=2.25]{summons-cropped.png}
	...
	\draw(0,-8mm)node[anchor=north,align=center, font=\mainTitleFont]{\uppercase{%
		Here follows a List of several Demons\\%
		and their Sigils of Summoning:}};

	\draw(Baseline)++(0,-18mm)node[anchor=north, font=\secTitleFont]{%
		\uppercase{Other Sigils of necissity include:}};
	\newrow
	...
	\draw(Baseline)++(0,-80mm)node[align=center, font=\bottomTitleFont]{\uppercase{%
